...
function calculateFoundScore(geocacheLogs) {
  ...
  // TODO figure out what enable listing and unarchive ar
  const isPositiveLog = log => log.LogType && log.LogType.WptLogTypeId === 2;

  const finds = geocacheLogs
    .map((l, i) => !isPositiveLog(l) * Math.pow(0.5, i))
    .reduce((s, x) => s - x, 1);
  return finds;
}
