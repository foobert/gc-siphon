...
function todoQuery(now) {
  return {
    $or: [
      { "api.Code": { $exists: false }, "api.IsPremium": { $ne: true } },
      {
...
async function processApi({ gcs }) {
  ...
  try {
    ...
    //metrics.gauge("fetch.todo", todoCount, ["source:groundspeak"]);
    metrics.gauge("fetch.limit", todayLimit, ["source:groundspeak"]);
    metrics.increment("fetch.count", fetchCount, ["source:groundspeak"]);
  } catch (err) {
