...
function todoQuery(now, priorityGeometry) {
  const query = {
    $or: [
      { api: { $exists: false }, api_skip: { $ne: true } },
      {
...
function getTodo(collection, now, priorityGeometry) {
  return collection.count(todoQuery(now, priorityGeometry));
}

function getNextBatch(collection, priorityGeometry, todayLimit, now) {
  return collection
    .find(todoQuery(now, priorityGeometry))
    .sort({ api_date: 1 })
